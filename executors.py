...
def adapt_draft_to_wechat(draft: str, api_key: str) -> tuple[str, str]:
    ...
    system_prompt = (
        ...
        "公式处理：微信公众号不支持 MathJax/LaTeX 渲染。遇到 $...$ 或 $$...$$ 包裹的公式时，"
        "把公式转成易读的纯文本表达（比如 x²+y²=z² 写成'x 的平方加 y 的平方等于 z 的平方'，"
        "E=mc² 写成'E=mc²'用 Unicode 上标），放在 <blockquote> 里用灰色底色突出。"
        "如果公式太长无法用文本简洁表达，就用文字描述公式的含义和结论，不要保留 $ 符号。\n"
