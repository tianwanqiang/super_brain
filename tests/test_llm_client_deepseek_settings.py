...
def test_configured_model_overrides_default(fake_config_path):
    ...
    assert settings["base_url"] == config_store.DEEPSEEK_BASE_URL_DEFAULT
    assert settings["max_tokens"] == config_store.DEEPSEEK_MAX_TOKENS_DEFAULT
...
def test_empty_string_fields_fall_back_to_defaults(fake_config_path):
    ...
    settings = llm_client.load_deepseek_settings()
    assert settings["model"] == config_store.DEEPSEEK_MODEL_DEFAULT
    assert settings["base_url"] == config_store.DEEPSEEK_BASE_URL_DEFAULT
...
def test_real_local_config_settings_are_loadable_without_error():
    """不 monkeypatch CONFIG_PATH——直接对这台机器上真实的 config.json 跑。
    ...
    """
    if not llm_client.CONFIG_PATH.exists():
        pytest.skip(
            f"{llm_client.CONFIG_PATH} 在这台机器上不存在（预期情况，比如 CI 云端"
